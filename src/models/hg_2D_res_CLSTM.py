...
# for now, it is hour glass for 2D pose
class HourglassLSTM(nn.Module):
    def __init__(self, nStack, nModules, nFeats, numOutput):
        ...
        for i in range(self.nStack):
            ...
            lin = nn.Sequential(nn.Conv2d(self.nFeats,self.nFeats,bis=True, kernel_size=1,stride=1),
                                    nn.BatchNorm2d(self.nFeats),
                                    self.relu)
            _lin_.append(lin)
            _tmpOut.append(nn.Conv2d(self.nFeats, self.numOutput,bias=True, kernel_size=1, stride=1))
            if i < self.nStack-1:
                _ll_.append(nn.Conv2d(nFeats,self.nFeats,bias=True, kernel_size=1, stride=1))
                _tmpOut_.append(nn.Conv2d(numOutput, self.nFeats, bias=True, kernel_size=1, stride=1))
